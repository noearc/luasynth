...
function defs.processSamplePair(state, l, r)
	...
	state.n = state.n + 1
	local outL = processInMono(state.dryOut, state.wetOut, state.feedback, state.bufL, state.n, state.sampleDelay, l)
	local outR = processInMono(state.dryOut, state.wetOut, state.feedback, state.bufR, state.n, state.sampleDelay, r)
	return outL, outR
end
